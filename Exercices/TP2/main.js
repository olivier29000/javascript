...
function verifierPrenom(prenom) {
    if (parseInt(jourDeNaissance)>31 
    || parseInt(jourDeNaissance<1)
    ||!Number.isInteger(parseInt(jourDeNaissance))) {
        return true;
    } else {
        return false;
    }
...
function demanderTaille() {
    taille=prompt('quel est ta taille en centimètres');

function verifierTaille(taille) {
    if (parseInt(taille)<200
    ||!Number.isInteger(parseInt(taille))) {
        return true;
    } else {
        return false;
    }
...
function demanderJourDeNaissance(){
    jour=prompt('quel est ton jour de naissance (de 1 à 31)');
}

function demanderDateDeNaissance() {
    while (!verifierJourDeNaissance(jour)) {
        demanderJourDeNaissance();
    }
    while (!verifierMoisDeNaissance(mois)) {
        demanderMoisDeNaissance();
    }
    while (!verifierAnneeDeNaissance(annee)) {
        demanderAnneeDeNaissance();
    }
    
}

while (!verifierPrenom(prenom)) {
    demanderPrenom();
}

while (!verifierTaille(taille)) {
    demanderTaille();
}

while (!verifierDateDeNaissance(jourDeNaissance)) {
